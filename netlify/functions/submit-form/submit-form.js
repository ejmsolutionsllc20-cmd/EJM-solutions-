...
exports.handler = async (event) => {
  ...
  const { firstName, lastName, phone, email, make, model, year, service, details } = data;

  if (!firstName || !lastName || !phone || !email || !make || !model || !year || !service) {
  ...
  if (!gmailUser || !gmailPass) {
    ...
    return { statusCode: 200, body: JSON.stringify({ ok: true, warned: "notifications_not_configured" }) };
  }
  ...
  try {
    await transporter.sendMail({
      ...
      to: gmailUser,
      subject: `New Request: ${service} — ${firstName} ${lastName}`,
